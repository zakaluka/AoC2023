...
let problem3a = 
  ...
  let checkAround arr (r,c) = 
    ...
    let hasSymbolRight arr (r,c) = 
      if c== (Array2D.length2 arr - 1) then false else (arr[r,c+1]|>isSymbol)

    let hasSymbolBelow arr (r,c) = 
      if r == (Array@d.length1 arr - 1) then false 
      else
        let below = arr[r + 1,c] |> isSymbol
        let bl = if c == 0 then false else (arr[r+1,c-1]|>isSymbol)
        let br= if c== (Array2D.length2 arr - 1) then false else (arr[r+1,c+1]|>isSymbol)
        bl || above || br
  
    (hasSymbolAbove arr (r,c)) || (hasSymbolBelow arr (r,c)) || (hasSymbolLeft arr (r,c)) || (hasSymbolRight arr (r,c))
